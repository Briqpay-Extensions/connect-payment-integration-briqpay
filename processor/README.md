...
- **Webhook-Driven Payment & Order Recovery**: When the buyer never returns to the storefront (e.g. off-site payment redirect), the session-less webhook creates a correctly-tagged Payment from the Checkout transaction-item id staged on the cart and stages pre-order session data so commercetools copies it onto the order at creation
- **Per-Cart Variant Selection**: Reads a merchant-set cart custom field (`briqpay-variant-id`) on every session creation and forwards it as `product.variantId`, so each cart can resolve to a different Briqpay checkout variant; absent the field, Briqpay uses the account default variant
...
| `BRIQPAY_CHECKOUT_TRANSACTION_ITEM_ID_KEY`              | Cart custom field name where the connector persists the Checkout transaction-item id on first session creation, so the session-less webhook can create a correctly-tagged Payment and let Checkout auto-create the Order when the buyer never returns.                                    | `briqpay-checkout-transaction-item-id`              |
| `BRIQPAY_VARIANT_ID_KEY`                                | Cart custom field name the merchant sets before checkout renders to select the Briqpay checkout variant for that cart. Read per session creation and forwarded as `product.variantId`; when unset, Briqpay uses the account default variant.                                              | `briqpay-variant-id`                                |
| `BRIQPAY_SYNCED_PAYLOAD_HASH_KEY`                       | Cart custom field name where the connector records a hash of the payload the Briqpay session already holds, so `/config` can skip an update Briqpay would treat as a no-op. Purely an optimisation — when the field is missing the connector updates on every checkout render, as before. | `briqpay-synced-payload-hash`                       |
...
> Please refer to the [commercetools documentation regarding scopes](https://docs.commercetools.com/api/scopes), names may change and names might be different from what you see in the commercetools GUI.
...
- `briqpay-checkout-transaction-item-id` - Checkout transaction-item id persisted on the cart at first session creation, overwritten on each new active Checkout session. Lets the session-less webhook create a correctly-tagged Payment and drive Order auto-creation when the buyer never returns. See the top-level README's "Webhook-Driven Payment & Order Recovery" section.
- `briqpay-variant-id` - Briqpay checkout variant id the merchant sets on the cart before checkout renders. Not written by the connector; read per session creation and forwarded as `product.variantId` so each cart can select a different Briqpay variant. See the top-level README's "Per-Cart Variant Selection" section.
