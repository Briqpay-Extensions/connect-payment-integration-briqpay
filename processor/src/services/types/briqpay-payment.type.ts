...
export interface RegularCartItem {
  ...
  /**
   * Unit price EXCLUDING VAT (minor units)
   * -> existing field, keep it
   */
  unitPrice: number

  /**
   * NEW: Unit price INCLUDING VAT (minor units)
   */
  unitPriceIncVat: number

  /**
   * VAT rate in basis points (e.g. 19% = 1900)
   */
  taxRate: number

  /**
   * Discount percentage in basis points (10% = 1000)
   */
  ...
  /**
   * NEW: Total VAT amount for the line.
   */
  totalVatAmount: number

  /**
   * NEW (optional): Product image URL
   */
  imageUrl?: string
}
